...
class ImageServiceTest {
	...
	class DeleteImage {
		...
		void deleteImageSuccess() {
			...
			when(imageRepository.findAllByReferenceIdAndImageType(referenceId, ORDER)).thenReturn(images);
			...
			// then
			verify(imageRepository).findAllByReferenceIdAndImageType(referenceId, MARKET);
			verify(imageRepository, times(images.size())).delete(any(Image.class));
		}
		...
		void deleteImageFailByNotFound() {
			// given
			Long referenceId = 1L;
			when(imageRepository.findAllByReferenceIdAndImageType(referenceId, MARKET)).thenReturn(emptyList());
